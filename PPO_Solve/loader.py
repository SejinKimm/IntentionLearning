...
class SizeConstrainedLoader(ARCLoader):
    def __init__(self, size, train=True) -> None:
        ...
        self.config_path = os.path.join(current_dir, 'ppo/ppo_config_intention.yaml')

        self._pathlist = []
        config = load_config(self.config_path)['train']
        for i in range(config['nexamples']):
            file_path = f"{config['example_task_path']}/{config['task_id']}/{config['task_id']}_{i}.json"
            self._pathlist.append(file_path)
        self.data = self.parse(train=train)
        print("!!!!!!!!!!!!!!!!!!!!!!", len(self.data), "augmented tasks are loaded")
    ...
    def parse(self, **kwargs):
        ...
        for p in self._pathlist:
            with open(p) as fp:
                ...
                for d in problem['ex_in']:
                    inp = np.array(d, dtype=np.uint8)
                    if inp.shape[0] > self.size or inp.shape[1] > self.size:
                        continue
                    ti.append(inp)

                for d in problem['ex_out']:
                    oup = np.array(d, dtype=np.uint8)
                    if oup.shape[0] > self.size or oup.shape[1] > self.size:
                        continue
                    to.append(oup)
                ...
                desc = {'id': os.path.basename(fp.name).split('_')[0], 'number': int(os.path.basename(fp.name).split('_')[1].split('.')[0])}
